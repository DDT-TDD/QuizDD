...
impl ContentSeeder {
    ...
    /// Seed content with aggressive backfill to ensure completeness
    /// Uses question count (not tag checks) to verify all tiers are present
    pub fn seed_if_empty(&self) -> AppResult<()> {
        let stats = self.get_content_statistics()?;
        let total_questions = stats.total_questions;
        
        // Expected minimum counts per tier:
        // - Base content (all subjects): ~1200-1400 questions
        // - Boost content (KS1+KS2 expansion): ~450-550 additional
        // - Ultra boost (extended variety): ~350-400 additional
        // Total target: 2100+ questions
        
        const BOOST_MINIMUM: u32 = 1600;  // Base + Boost minimum
        const ULTRA_MINIMUM: u32 = 2000;  // All three tiers minimum
        
        if total_questions == 0 {
            println!("✓ Database is empty, seeding with comprehensive educational content (BASE + BOOST + ULTRA)...");
            self.seed_all_content()?;
        } else if total_questions < ULTRA_MINIMUM {
            println!("⚠️  Database has {} questions (target: 2100+)", total_questions);
            println!("✓ Detected incomplete content, checking which tiers are missing...");
            ...
            // If we have less than BOOST_MINIMUM, add boost content
            if total_questions < BOOST_MINIMUM {
                println!("  → Adding KS1/KS2 boost content (estimated +500 questions)...");
                self.seed_ks1_boost_content(&subject_map)?;
                self.seed_ks2_boost_content(&subject_map)?;
            }
            
            // Always add ultra boost if below target
            println!("  → Adding ultra boost content (estimated +350 questions)...");
            self.seed_ultra_boost_content(&subject_map)?;
            
            let stats_after = self.get_content_statistics()?;
            println!("✅ Backfill completed: {} → {} total questions", total_questions, stats_after.total_questions);
        } else {
            println!("✅ Database is complete with {} questions (target: 2100+)", total_questions);
        }
